...
def run():
	...
	for (grad, var), (capped_grad, _) in zip(gvs, capped_gvs):
		if grad is not None:
			...
			tf.summary.histogram('capped_fraction/{}'.format(var.name), tf.nn.zero_fraction(grad - capped_grad))
	...
	with tf.Session() as sess:
		...
		for step in range(1, FLAGS.max_steps + 1):
			...
			sess.run(
				[train_op], feed_dict={X: batch_x, Y: batch_y, training: True})
			if step % display_step == 0 or step == 1:
				...
				summary, loss, acc = sess.run(
					...
					feed_dict={X: valid_x, Y: valid_y, training: False})
				print("Step " + str(step) + ", Minibatch Loss= " + "{:.4f}".format(loss) +
		...
		for step in range(1, 1000):
			...
			# Calculate batch loss and accuracy
			summary, loss, acc = sess.run(
				[merged, loss_op, accuracy], feed_dict={X: test_data, Y: test_label, training: False})
			test_loss += loss
			test_acc += acc
